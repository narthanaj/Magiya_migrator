...
class MagiyaMigration:
    ...
    def pre_migration_checks(self):
        ...
        if hasattr(self, 'migrate_users') and self.migrate_users:
            ...
            print(f"  {Fore.CYAN}Gender distribution:")
        ...
        if hasattr(self, 'migrate_users') and self.migrate_users:
            print(f"  - Users table: {self.stats['users']['total_records']} records")
        if self.migrate_addresses:
    ...
    def post_migration_validation(self):
        ...
        # Final status
        total_failed = sum(stats['failed_records'] for stats in self.stats.values())
        if total_failed == 0:
            print(f"\n{Fore.GREEN}✓ Migration completed successfully!")
        else:
            print(f"\n{Fore.YELLOW}⚠ Migration completed with {total_failed} total failures")
